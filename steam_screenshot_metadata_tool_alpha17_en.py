...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title('Steam Screenshot Metadata Tool alpha 17')
    ...
    def _build_ui(self):
        ...
        self.tree.heading('entry', text='Entry', command=lambda: self.sort_tree('entry'))
        self.tree.heading('game', text='Game / AppID', command=lambda: self.sort_tree('game'))
        self.tree.heading('status', text='Status', command=lambda: self.sort_tree('status'))
        self.tree.heading('filename', text='Filename', command=lambda: self.sort_tree('filename'))
        self.tree.heading('location', text='Metadata', command=lambda: self.sort_tree('location'))
        self.tree.heading('created', text='Created', command=lambda: self.sort_tree('created'))
        ...
        ttk.Label(meta_frame, text='Current location metadata').pack(anchor='w', pady=(0, 1))
        ttk.Entry(meta_frame, textvariable=self.current_location_var, state='readonly').pack(fill='x')

        ttk.Label(meta_frame, text='New location metadata').pack(anchor='w', pady=(3, 0))
        ...
        self.info_var = tk.StringVar(value='Ready. The default editable field currently writes the Location metadata in screenshots.vdf. Backups are optional and manual.')
        ...
        ttk.Label(
            footer,
            text='The default editable field currently targets the Location metadata in screenshots.vdf. Counters refer to the selected game.'
        ).pack(anchor='w')
    ...
    def sort_entries_for_column(self, entries, column, descending):
        def sort_key(entry):
            data = entry['data']
            if column == 'entry':
                try:
                    return int(entry['entry_key'])
                except Exception:
                    return entry['entry_key']
            if column == 'game':
                return data.get('gameid', '').strip()
            if column == 'status':
                return entry_status(data)
            if column == 'filename':
                return data.get('filename', '').lower()
            if column == 'location':
                return data.get('location', '').lower()
            if column == 'created':
                return data.get('creation', '')
            return ''

        entries.sort(key=sort_key, reverse=descending)
    ...
    def sort_tree(self, column):
        if self.sort_column == column:
            ...
            self.sort_desc = (column == 'created')

        self.sort_entries_for_column(self.filtered_entries, column, self.sort_desc)
        self.fill_tree()
    ...
    def load_vdf(self):
        ...
        if not path or not path.exists():
            messagebox.showerror('Error', 'Please select a valid screenshots.vdf file.')
            return
        ...
        self.refresh_list()
        self.info_var.set(f'{len(self.entries)} screenshot entries loaded.')
    ...
    def on_select(self, event=None):
        ...
        self.selected_game_var.set(f'Selected game: {app_label(gameid)}')
    ...
    def save_current(self, create_backup=True):
        entry = self.get_selected_entry()
        if not entry or not self.vdf_path:
        ...
        try:
            backup = make_backup(self.vdf_path) if create_backup else None
            ...
            if backup is not None:
                ...
                self.info_var.set('Location metadata applied without creating a backup.')
                self.play_feedback('apply')
        except Exception as e:
